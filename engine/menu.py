...
class Menu():
    def __init__(self, scr_id=1, fps_id=1):
        ...
        pygame.mixer.music.load('sound/song1.wav')
    ...
    def set_menu(self):
        ...
        control_title_text = self.fontT.render('CONTROL', True, (255,255,255))
        self.control_title = pygame.Surface((control_title_text.get_width()+20*sw,control_title_text.get_height()+10*sh))
        self.control_title.fill((40,50,150))
        self.control_title.blit(control_title_text, self.control_title.get_rect(center=(self.control_title.get_width()//2+12*sw,self.control_title.get_height()//2+8*sh)))
        self.control_title_rect = self.control_title.get_rect(midleft=(self.scr_size[0]//2,80*sh))
        self.control_surf = pygame.Surface((450*sw,453*sh), pygame.SRCALPHA)
        control_pic = pygame.image.load('sprite/control.png').convert_alpha()
        control_pic = pygame.transform.smoothscale(control_pic, (control_pic.get_width()*sw, control_pic.get_height()*sh))
        self.control_surf.blit(control_pic, (0,0))
        for i, text in enumerate(['Movement','Jump','Dash','Go back']):
            this_text = self.fontL.render(text, True, (255,255,255))
            text_surf = pygame.Surface((this_text.get_width()+20*sw,this_text.get_height()+10*sh))
            text_surf.fill((40,50,150))
            text_surf.blit(this_text, text_surf.get_rect(center=(text_surf.get_width()//2+10*sw,text_surf.get_height()//2+6*sh)))
            self.control_surf.blit(text_surf, (260*sw,(94+102*i)*sh))
        self.control_surf_rect = self.control_surf.get_rect(topleft=(self.scr_size[0]//2,140*sh))
    ...
    def run_menu(self, event_list, dt):
        if self.id == 1 and self.new_game:
            ...
            pygame.mixer.music.load('sound/song2.wav')
    ...
    def option_menu(self, event_list, _):
        ...
        self.screen.blit(self.control_title, self.control_title_rect)
        self.screen.blit(self.control_surf, self.control_surf_rect)
        self.back_button.draw(self.screen)
